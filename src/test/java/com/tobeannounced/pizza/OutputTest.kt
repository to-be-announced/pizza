...
class OutputTest {
    ...
    @Test
    fun shouldCreateOutputFromSolution() {
        val solution = emptySet<Set<Pair<Int, Int>>>()
        assertEquals("0", outputFrom(solution))
    }

    @Test
    fun shouldCreateOutputFromSolutionForSingleSlice() {
        val slice = setOf(0 to 0, 0 to 1)
        val solution = setOf(slice)
        assertEquals("1\n" +
                "0 0 0 1", outputFrom(solution))
    }

    @Test
    fun shouldCreateOutputFromSolutionForMultipleSlices() {
